...
from appv1.schemas.user import UserUpdate, UserCreate
...
def get_all_users_by_rol(db: Session, user_rol: str):
    try:
        sql = text("SELECT * from users where user_role = :user_rol ")
        result = db.execute(sql, {"user_rol": user_rol}).fetchall()
        return result
    except SQLAlchemyError as e:
